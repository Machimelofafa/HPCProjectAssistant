(function () {
  ...
  function createCPMWorker() {
    if (location.protocol === 'file:') {
      ...
      if (src) {
        const blob = new Blob([src.textContent], {
  ...
  const SettingsStore = function () {
    ...
    const listeners = {};
    function save() {
      localStorage.setItem('hpc-settings', JSON.stringify(state));
    }
    ...
    function set(part) {
      ...
      save();
      emit('settings:changed', state);
    }
    function setFilters(part) {
      state.filters = Object.assign(state.filters, part);
      save();
      emit('filters:changed', state.filters);
      emit('settings:changed', state);
    }
    function setCalendar(part) {
      state.calendar = Object.assign(state.calendar, part);
      save();
      emit('settings:changed', state);
    }
    return {
  ...
  function validateProject(project) {
    ...
    if (!project || typeof project !== 'object') {
      errors.push({
        sev: 'critical',
        msg: 'Invalid project file format. Expected a JSON object.'
      });
      return {
        ok: false,
        errors,
    ...
    if (!projectVersion) {
      warnings.push({
        sev: 'warn',
        msg: 'No schemaVersion found. Assuming older format and attempting to migrate.'
      });
    ...
    if (project.schemaVersion < SCHEMA_VERSION) {
      ...
      if (project.tasks && Array.isArray(project.tasks)) {
        ...
        warnings.push({
          sev: 'info',
          msg: 'Project migrated to schema v1.0.0: ensured all tasks have an "active" status.'
        });
      }
      ...
      errors.push({
        sev: 'critical',
        msg: "Project schema version (".concat(project.schemaVersion, ") is newer than this application's supported version (").concat(SCHEMA_VERSION, "). Please update the application.")
      });
      return {
        ok: false,
        errors,
    ...
    if (!project.startDate || !/^\d{2}-\d{2}-\d{4}$/.test(project.startDate)) {
      errors.push({
        sev: 'error',
        msg: 'Project is missing a valid "startDate".'
      });
      project.startDate = todayStr(); // Attempt to fix
      warnings.push({
        sev: 'warn',
        msg: 'Project "startDate" was missing or invalid. It has been reset to today.'
      });
    }
    if (!project.tasks || !Array.isArray(project.tasks)) {
      errors.push({
        sev: 'critical',
        msg: 'Project is missing a valid "tasks" array.'
      });
      return {
        ok: false,
        errors,
    ...
    for (const task of project.tasks) {
      if (!task.id) {
        warnings.push({
          sev: 'warn',
          msg: "A task is missing an ID. A new one will be generated."
        });
        task.id = uid('t');
        migrated = true;
      }
      if (taskIds.has(task.id)) {
        warnings.push({
          sev: 'warn',
          msg: "Duplicate task ID found: ".concat(task.id, ". A new ID will be generated.")
        });
        task.id = uid('t');
        migrated = true;
      }
  ...
  // ---------------------------- dependency parser ----------------------------
  function adjustIncomingLags(task, delta) {
    const out = [];
  ...
  const SM = function () {
    ...
    function updateUndoUI() {
      ...
      if (undoBtn) {
        ...
        undoBtn.title = lastUndo ? "Undo: ".concat(lastUndo.name, " (Ctrl+Z)") : 'Undo (Ctrl+Z)';
      }
      if (redoBtn) {
        const lastRedo = redo[redo.length - 1];
        redoBtn.disabled = !canRedo();
        redoBtn.title = lastRedo ? "Redo: ".concat(lastRedo.name, " (Ctrl+Y)") : 'Redo (Ctrl+Y)';
      }
    }
    function saveState(state) {
      try {
        ...
        if (lastSavedBadge) {
          lastSavedBadge.innerHTML = "<span class=\"pill-icon\" aria-hidden=\"true\">\uD83D\uDCBE</span> Saved: ".concat(lastSaved);
        }
      } catch (e) {
  ...
  const WarningEngine = function () {
    ...
    function checkTaskProperties(project, cpm, warnings, FIX) {
      ...
      if (dupIds.size > 0) {
        push('critical', "Duplicate task ID(s) found: ".concat(Array.from(dupIds).join(', ')), {
          fix: () => {
            const s = SM.get();
            const used = new Set();
            s.tasks.forEach(t => {
      ...
      for (const t of project.tasks) {
        if (!t.id || String(t.id).trim() === '') {
          push('error', "Task found with missing ID.", {
            taskId: t.id,
            fix: () => {
              SM.updateTask(t.id, {
                id: uid('t')
              }, {
                name: 'Assign Missing ID'
              });
            }
          });
        }
        if (!t.name || String(t.name).trim() === '') {
          push('error', "Task has a missing name.", {
            taskId: t.id,
            fix: () => {
              SM.updateTask(t.id, {
                name: "Task ".concat(t.id)
              }, {
                name: 'Assign Missing Name'
              });
            }
          });
        }
        ...
        if (pd.error) {
          push('error', "Invalid duration: ".concat(pd.error), {
            taskId: t.id,
            fix: () => {
              SM.updateTask(t.id, {
                duration: 1
              }, {
                name: 'Fix Invalid Duration'
              });
            }
          });
        }
    ...
    function checkDependencies(project, cpm, warnings, FIX) {
      ...
      for (const t of project.tasks) {
        ...
        for (const tok of depStrings) {
          ...
          if (!d) continue;
          if (d.pred === t.id) {
            push('critical', "Self-dependency is not allowed.", {
              taskId: t.id,
              fix: () => {
                SM.updateTask(t.id, {
                  deps: (t.deps || []).filter(x => x !== tok)
                }, {
                  name: 'Remove Self-Dependency'
                });
              }
            });
          }
          if (!id2[d.pred]) {
            push('error', "Links to missing dependency: \"".concat(d.pred, "\"."), {
              taskId: t.id,
              fix: () => {
                SM.addTasks([{
                  id: d.pred,
                  name: "New: ".concat(d.pred),
                  duration: 1,
                  deps: []
                }], {
          ...
          if (seen.has(d.pred)) {
            push('warn', "Duplicate dependency on \"".concat(d.pred, "\"."), {
              taskId: t.id,
              fix: () => {
                const s = SM.get();
                ...
                SM.replaceTasks(s.tasks, {
                  name: 'Remove Duplicate Dependency'
                });
              }
            });
          }
          ...
          if (id2[d.pred] && id2[d.pred].active === false) {
            push('warn', "Predecessor \"".concat(d.pred, "\" is inactive."), {
              taskId: t.id,
              fix: () => {
                SM.updateTask(d.pred, {
                  active: true
                }, {
                  name: 'Activate Predecessor'
                });
              }
            });
          }
      ...
      for (const t of cpm.tasks) {
        ...
        if (cur.startConstraint && cur.startConstraint.type === 'MSO') {
          ...
          if (esReq > (cur.startConstraint.day | 0)) {
            push('error', "MSO violated: must start on/after day ".concat(esReq, ", but set to ").concat(cur.startConstraint.day, "."), {
              taskId: t.id,
              fix: () => {
                SM.updateTask(t.id, {
                  startConstraint: {
        ...
        if (dur === 0 && !isMilestone) {
          push('info', "Zero-duration task. Consider making it a milestone or setting duration to 1d.", {
            taskId: t.id,
            fix: () => {
              SM.updateTask(t.id, {
                duration: 1
              }, {
                name: 'Set Duration to 1d'
              });
            }
          });
        }
    ...
    function run(project, cpm) {
      ...
      for (const w of warnings) {
        const key = "".concat(w.sev, "|").concat(w.msg, "|").concat(w.taskId || '');
        if (!seen.has(key)) {
          seen.add(key);
  ...
  function getActiveSubsystems() {
    return $$('#subsysFilters input[type="checkbox"]').filter(x => x.checked).map(x => x.value);
  }
  ...
  function renderGraph(project, cpm) {
    ...
    for (const t of tasks) {
      ...
      for (const e of edges) {
        ...
        line.setAttribute('d', "M ".concat(p1.x + p1.width, " ").concat(p1.y + p1.height / 2, " L ").concat(p2.x - 12, " ").concat(p2.y + p2.height / 2));
    ...
    for (const t of tasks) {
      ...
      rect.setAttribute('style', "stroke:".concat(color));
      ...
      metaText.textContent = "".concat(esc(t.phase || ''), " \u2022 ").concat(esc(String(t.duration)), "d \u2022 slack ").concat(esc(String(t.slack)));
  ...
  function renderGantt(project, cpm) {
    ...
    svg.setAttribute('viewBox', "0 0 ".concat(W, " ").concat(chartH));
    ...
    for (const d of tickDates) {
      const off = cal.diff(startDate, d);
      const x = scale(off);
      ...
      t.setAttribute('y', 14);
      t.textContent = "".concat(MONTHS[d.getMonth()], " ").concat(d.getFullYear());
      gAxis.appendChild(t);
    }
    ...
    rows.forEach(r => {
      ...
      const labelText = "".concat(t.name, ", phase ").concat(t.phase || 'N/A', ", duration ").concat(durVal, " days, ").concat(t.critical ? 'critical path' : 'slack ' + t.slack + ' days');
      ...
      if (isMilestone) {
        ...
        diamond.setAttribute("transform", "rotate(45 ".concat(x, " ").concat(y + 8, ")"));
        diamond.setAttribute("class", "milestone" + (t.critical ? ' critical' : ''));
        diamond.setAttribute("style", "stroke:".concat(col));
        ...
        rect.setAttribute("style", "stroke:".concat(col));
    ...
    if (summaryContainer) {
      ...
      for (const r of rows) {
        ...
        const durVal = parseDuration(t.duration).days || 0;
        li.innerHTML = "Task: <strong>".concat(esc(t.name), "</strong> (Phase: ").concat(esc(t.phase) || 'N/A', ").\n        <span class=\"duration\">Duration: ").concat(durVal, " days</span>.\n        <span class=\"slack\">Slack: ").concat(t.slack, " days</span>.\n        Status: ").concat(t.critical ? 'Critical' : 'Not Critical', ".");
        list.appendChild(li);
      }
  ...
  function calcEarliestESFor(project, id, dur) {
    ...
    if (!cur) return 0;
    let es = 0;
    for (const e of normalizeDeps(cur)) {
      const p = id2[e.pred];
      if (!p) continue;
      const pd = parseDuration(p.duration).days || 0;
      if (e.type === 'FS') es = Math.max(es, (p.ef != null ? p.ef : pd) + e.lag);else if (e.type === 'SS') es = Math.max(es, (p.es != null ? p.es : 0) + e.lag);else if (e.type === 'FF') es = Math.max(es, (p.ef != null ? p.ef : pd) + e.lag - dur);else if (e.type === 'SF') es = Math.max(es, (p.es != null ? p.es : 0) + e.lag - dur);
    }
    if (cur.startConstraint) {
      const sc = cur.startConstraint;
      if (sc.type === 'SNET' || sc.type === 'MSO') es = Math.max(es, sc.day | 0);
    }
    if (cur.fixedStart != null) es = Math.max(es, cur.fixedStart | 0);
    return Math.max(0, Math.round(es));
  }
  function onTimelinePointerDown(ev) {
    const svg = ganttState.svg;
    if (!svg) return;
    const tgt = ev.target;
    ...
    drag = {
      id: gg.getAttribute('data-id'),
      side: h ? h.getAttribute('data-side') : 'move',
      x0: +rect.getAttribute('x'),
      w0: +rect.getAttribute('width'),
      px0: ev.clientX,
      py0: ev.clientY,
      moved: false
    };
  ...
  function onTimelinePointerMove(ev) {
    ...
    const dx = ev.clientX - drag.px0,
      dy = ev.clientY - drag.py0;
    if (!drag.moved) {
      if (Math.abs(dx) < 2 && Math.abs(dy) < 2) return;
      drag.moved = true;
      svg.setPointerCapture(ev.pointerId);
    }
    const gg = $(".bar[data-id=\"".concat(drag.id, "\"]"), svg);
    const rect = gg.querySelector('rect');
    ...
    if (drag.side === 'right') {
      ...
      const newX = Math.max(ganttState.P, drag.x0 + dx);
      ...
      if (ok) {
        ...
        showHint(ev.clientX, ev.clientY, "Blocked: earliest ".concat(allowed, "d"));
      }
    }
  }
    ...
    if (drag.side === 'right') {
      ...
      if (esNew < allowed && !autoLag) {
        ...
        SM.updateTask(drag.id, {
          ...
          record: true,
          name: 'Snap to Earliest'
        });
        showToast("Snapped to earliest ".concat(allowed, "d"));
        ...
        if (autoLag) {
          ...
          SM.replaceTasks(s.tasks, {
            record: true,
            name: "Adjust Lags by ".concat(delta, "d")
          });
          showToast("Adjusted predecessor lags by ".concat(delta, "d"));
        } else {
          const sc = {
            type: 'SNET',
            day: Math.max(0, esNew)
          };
          SM.updateTask(drag.id, {
            startConstraint: sc
          }, {
            record: true,
            name: 'Add SNET Constraint'
          });
  ...
  function renderFocus(project, cpm) {
    ...
    for (const t of near) {
      ...
      row.className = 'row';
      row.innerHTML = "<span>".concat(esc(t.name), "</span><span class=\"slack\">slack ").concat(t.slack, "d</span>");
      L.appendChild(row);
    }
  ...
  function renderIssues(project, cpm, targetSel) {
    ...
    if (badgesContainer) {
      badgesContainer.innerHTML = ['critical', 'error', 'warn', 'info'].map(sev => {
        if (counts[sev] > 0) {
          return "<span class=\"badge sev-".concat(sev, "\" style=\"background:var(--").concat(sev, ");color:white;margin-left:4px;border-radius:6px;padding:2px 6px;font-size:0.8em;\" title=\"").concat(counts[sev], " ").concat(sev, " issues\">").concat(counts[sev], "</span>");
        }
        return '';
      }).join('');
    }
    ...
    for (const [msg, items] of groups) {
      ...
      summary.className = "issue sev-".concat(sev);
      summary.style.display = 'grid';
      summary.style.gridTemplateColumns = '1fr auto';
      summary.style.alignItems = 'center';
      const badge = "<span class=\"badge\" style=\"background:var(--".concat(sev, ");color:white;margin-left:8px;border-radius:6px;padding:2px 6px;font-size:0.8em;\">").concat(items.length, "</span>");
      const fixButtonGroup = hasFix && !targetSel && items.length > 1 ? "<button class=\"btn small fix-btn\" data-fix-group=\"".concat(esc(msg), "\">Fix All</button>") : '';
      summary.innerHTML = "\n            <div class=\"msg\">".concat(esc(msg), " ").concat(badge, "</div>\n            <div class=\"actions\">").concat(fixButtonGroup, "</div>\n        ");
      ...
      for (const it of items) {
        ...
        const fixBtn = it.hasFix && !targetSel ? "<button class=\"btn small fix-btn\" data-i=\"".concat(it.id, "\">Fix</button>") : '';
        row.innerHTML = "\n                <span class=\"meta\" style=\"cursor:pointer; text-decoration:underline;\" data-task-id=\"".concat(it.taskId, "\">Task: ").concat(esc(taskName), "</span>\n                <div class=\"actions\">").concat(fixBtn, "</div>\n            ");
    ...
    if (!targetSel) {
      box.onclick = e => {
        ...
        if (b) {
          ...
          if (groupId) {
            ...
            if (fixedCount > 0) {
              showToast("Applied ".concat(fixedCount, " fix(es)."));
              refresh();
            }
          } else {
        ...
        if (taskLink) {
          ...
          if (taskId) {
            selectOnly(taskId);
            refresh();
            const bar = document.querySelector(".bar[data-id=\"".concat(taskId, "\"]"));
            if (bar) {
              bar.scrollIntoView({
                behavior: 'smooth',
                block: 'center'
      ...
      $('#btnAutoFix').onclick = () => {
        ...
        if (fixedCount > 0) {
          showToast("Auto-fix pass finished, applied ".concat(fixedCount, " fixes."));
          refresh();
        } else {
  ...
  function renderTaskTable(project, cpm) {
    ...
    for (const t of tasks) {
      ...
      row.innerHTML = "<td>".concat(esc(t.id), "</td><td>").concat(esc(t.name), "</td><td>").concat(dur, "</td><td>").concat(deps, "</td><td>").concat(esc(t.subsystem || ''), "</td><td>").concat(esc(t.phase || ''), "</td><td>").concat(t.pct || 0, "</td><td>").concat(t.critical ? '✓' : '', "</td><td>").concat(dur === 0 ? '✓' : '', "</td>");
  ...
  function renderContextPanel(selectedId) {
    ...
    const html = "\n    <h3 class=\"flex justify-between items-center\" style=\"margin-bottom: var(--space-4);\">\n      <span>Task Details</span>\n      ".concat(task.critical ? '<span class="badge" style="background:var(--crit); color:white;">Critical</span>' : '', "\n    </h3>\n    <div class=\"context-panel-content\" aria-live=\"polite\">\n      <div class=\"card\" style=\"margin-bottom: var(--space-4);\">\n        <div class=\"card-content\">\n          <div class=\"card-label\" style=\"font-weight: 700; color: var(--c-text); margin-bottom: var(--space-1);\">").concat(esc(task.name), "</div>\n          <code style=\"color: var(--c-text-muted); font-size: 0.8em;\">ID: ").concat(esc(task.id), "</code>\n        </div>\n      </div>\n\n      <div class=\"list\" style=\"display: grid; gap: var(--space-2); margin-bottom: var(--space-6);\">\n        <div class=\"row\" style=\"justify-content: space-between;\"><strong>Duration</strong> <span class=\"badge\">").concat(duration, " days</span></div>\n        <div class=\"row\" style=\"justify-content: space-between;\"><strong>Slack</strong> <span class=\"badge\">").concat(task.slack, " days</span></div>\n        <div class=\"row\" style=\"justify-content: space-between;\"><strong>Status</strong> <span>").concat(task.active !== false ? 'Active' : 'Inactive', "</span></div>\n        <div class=\"row\" style=\"flex-direction: column; align-items: flex-start; border-top: 1px solid var(--c-border); padding-top: var(--space-2); margin-top: var(--space-2);\">\n          <strong>Dependencies</strong>\n          <code style=\"word-break: break-all; background: var(--c-bg); padding: var(--space-2); border-radius: var(--radius-sm); margin-top: var(--space-1); width: 100%;\">").concat(esc(deps), "</code>\n        </div>\n      </div>\n\n      <h4 style=\"font-size: 0.9em; text-transform: uppercase; color: var(--c-text-muted); border-bottom: 1px solid var(--c-border); padding-bottom: var(--space-1); margin-bottom: var(--space-3);\">CPM Timings (days)</h4>\n      <div class=\"cards\" style=\"grid-template-columns: 1fr 1fr; gap: var(--space-2); font-size: var(--font-size-sm); margin-bottom: var(--space-6);\">\n          <div class=\"card metric-card\" style=\"padding: var(--space-2); text-align: center;\"><div class=\"card-content\"><div class=\"card-label\">ES</div><div class=\"metric\" style=\"font-size: 1.2rem;\">").concat(task.es, "</div></div></div>\n          <div class=\"card metric-card\" style=\"padding: var(--space-2); text-align: center;\"><div class=\"card-content\"><div class=\"card-label\">EF</div><div class=\"metric\" style=\"font-size: 1.2rem;\">").concat(task.ef, "</div></div></div>\n          <div class=\"card metric-card\" style=\"padding: var(--space-2); text-align: center;\"><div class=\"card-content\"><div class=\"card-label\">LS</div><div class=\"metric\" style=\"font-size: 1.2rem;\">").concat(task.ls, "</div></div></div>\n          <div class=\"card metric-card\" style=\"padding: var(--space-2); text-align: center;\"><div class=\"card-content\"><div class=\"card-label\">LF</div><div class=\"metric\" style=\"font-size: 1.2rem;\">").concat(task.lf, "</div></div></div>\n      </div>\n\n      <div id=\"context-panel-warnings\"></div>\n\n      <h4 style=\"font-size: 0.9em; text-transform: uppercase; color: var(--c-text-muted); border-bottom: 1px solid var(--c-border); padding-bottom: var(--space-1); margin-bottom: var(--space-3);\">Actions</h4>\n      <div class=\"button-group\" style=\"display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-2);\">\n        <button class=\"btn\" id=\"ctx-btn-edit\" title=\"Focus the editor fields for this task in the left sidebar\">Edit</button>\n        <button class=\"btn\" id=\"ctx-btn-duplicate\">Duplicate</button>\n        <button class=\"btn\" id=\"ctx-btn-toggle-active\">").concat(activeBtnText, "</button>\n        <button class=\"btn error\" id=\"ctx-btn-delete\">Delete</button>\n      </div>\n    </div>\n  ");
    ...
    $('#ctx-btn-toggle-active').addEventListener('click', () => {
      ...
      SM.updateTask(task.id, {
        active: !currentStatus
      }, {
        name: currentStatus ? 'Deactivate Task' : 'Activate Task'
      });
      refresh();
    });
    $('#ctx-btn-delete').addEventListener('click', () => {
      if (confirm("Are you sure you want to delete task \"".concat(task.name, "\"?"))) {
        selectOnly(task.id);
        deleteSelected();
      }
    });
  ...
  const SC = function () {
    ...
    function saveAs(name) {
      scenarios[name] = clone(snapshot());
      current = name;
      updateScenarioUI();
      showToast("Saved as \"".concat(name, "\""));
    }
    function switchTo(name) {
      if (!scenarios[name]) return;
      SM.set(clone(scenarios[name]), {
        name: "Switch to ".concat(name)
      });
      current = name;
      updateScenarioUI();
      showToast("Switched to \"".concat(name, "\""));
    }
    function get(name) {
      return scenarios[name] ? clone(scenarios[name]) : null;
    }
    function list() {
  ...
  function updateScenarioUI() {
    ...
    if (sel) {
      ...
      if (badge) badge.textContent = "Scenario: ".concat(SC.current());
    }
  }
  ...
  function buildCompare() {
    ...
    header.innerHTML = "<div class=\"msg\"><b>".concat(esc(meta ? meta.name : 'Baseline'), "</b> \u2022 \u0394finish ").concat(delta, "d \u2022 \u0394critical ").concat(critDelta, "</div>");
    ...
    rows.forEach(r => {
      ...
      div.className = 'row';
      div.innerHTML = "<span>".concat(esc(r.name), "</span><span class=\"slack\">\u0394start ").concat(r.ds, "d \u2022 \u0394finish ").concat(r.df, "d \u2022 \u0394slack ").concat(r.dsl, "d</span>");
      L.appendChild(div);
    });
  }
  ...
  async function openFile() {
    ...
    return new Promise(res => {
      ...
      inp.onchange = async () => {
        const f = inp.files[0];
        const txt = await f.text();
        if (f.name.endsWith('.csv')) res(csvToProject(txt));else res(JSON.parse(txt));
      };
      inp.click();
    });
  }
  function exportCSV() {
    ...
    for (const t of SM.get().tasks) {
      ...
      rows.push([t.id, t.name, parseDuration(t.duration).days || 0, (t.deps || []).join(' '), t.phase || '', t.subsystem || ''].map(x => "\"".concat(String(x).replaceAll('"', '""'), "\"")));
    }
  ...
  function csvToProject(csv) {
    ...
    const tasks = rows.map(r => {
      ...
      const d = parseDuration(+cols[iDur] || cols[iDur] || '');
      return {
        id: cols[iId] || uid('t'),
        name: cols[iName],
  ...
  function moveSelection(dir) {
    ...
    if (idx === -1) idx = dir > 0 ? 0 : tasks.length - 1;else idx = (idx + dir + tasks.length) % tasks.length;
  ...
  function deleteSelected() {
    ...
    SM.replaceTasks(remaining, {
      name: "Delete ".concat(ids.size, " Task(s)")
    });
    clearSelection();
    showToast("Deleted ".concat(ids.size, " task").concat(ids.size > 1 ? 's' : ''));
    refresh();
  }
  function duplicateSelected() {
    ...
    SM.replaceTasks(tasks, {
      name: "Duplicate ".concat(clones.length, " Task(s)")
    });
    SEL.clear();
    clones.forEach(c => SEL.add(c.id));
    LAST_SEL = clones.length ? clones[clones.length - 1].id : null;
    updateSelBadge();
    showToast("Duplicated ".concat(clones.length, " task").concat(clones.length > 1 ? 's' : ''));
    refresh();
  }
  ...
  function renderInlineEditor() {
    rafBatch(() => {
      ...
      for (const id of SEL) {
        ...
        row.className = 'row';
        const dur = parseDuration(t.duration).days || 0;
        row.innerHTML = "<input type=\"text\" data-id=\"".concat(id, "\" data-field=\"name\" value=\"").concat(esc(t.name), "\">\n      <input type=\"number\" min=\"0\" data-id=\"").concat(id, "\" data-field=\"duration\" value=\"").concat(dur, "\">\n      <input type=\"number\" min=\"0\" max=\"100\" data-id=\"").concat(id, "\" data-field=\"pct\" value=\"").concat(t.pct || 0, "\">\n      <input type=\"text\" data-id=\"").concat(id, "\" data-field=\"phase\" value=\"").concat(esc(t.phase || ''), "\">\n      <select data-id=\"").concat(id, "\" data-field=\"subsystem\">").concat(SUBS.map(sub => "<option".concat(sub === t.subsystem ? ' selected' : '', ">").concat(esc(sub), "</option>")).join(''), "</select>\n      <select data-id=\"").concat(id, "\" data-field=\"active\"><option value=\"true\"").concat(t.active !== false ? ' selected' : '', ">true</option><option value=\"false\"").concat(t.active === false ? ' selected' : '', ">false</option></select>");
        box.appendChild(row);
      }
      box.querySelectorAll('input,select').forEach(inp => {
        inp.addEventListener('change', () => {
          ...
          let val = inp.value;
          if (field === 'duration') val = parseInt(val, 10) || 0;else if (field === 'active') val = val === 'true';else if (field === 'pct') val = Math.min(100, Math.max(0, parseInt(val, 10) || 0));
          SM.updateTask(id, {
            [field]: val
          }, {
            name: "Edit ".concat(field)
          });
          refresh();
        });
      });
  ...
  function updateSelBadge() {
    rafBatch(() => {
      setText($('#selBadge'), "".concat(SEL.size, " selected"));
  ...
  function applyBulk() {
    ...
    for (const t of s.tasks) {
      ...
      if (durV !== '') {
        ...
        if ($('#bulkDurMode').value === 'set') t.duration = n;else t.duration = Math.max(0, parseDuration(t.duration).days + n);
      }
    ...
    SM.replaceTasks(s.tasks, {
      name: "Bulk Edit ".concat(changed, " task(s)")
    });
    showToast("Bulk applied to ".concat(changed, " tasks"));
    refresh();
  }
  ...
  function insertTemplate(which) {
    ...
    SM.addTasks(toAdd, {
      name: "Insert ".concat(which, " Template")
    });
    showToast("Inserted ".concat(toAdd.length, " tasks from ").concat(which));
  }
  ...
  function setupLegend() {
    ...
    box.innerHTML = '';
    SUBS.forEach(name => {
      const cls = slug(name).replace('_', '');
      const div = document.createElement('label');
      div.className = 'tag';
      div.innerHTML = "<input type=\"checkbox\" checked value=\"".concat(name, "\"><span class=\"dot ").concat(cls, "\" style=\"background:").concat(colorFor(name), "\"></span> ").concat(name);
      box.appendChild(div);
    });
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    try {
      ...
      // controls
      $('#slackThreshold').onchange = () => {
        SettingsStore.set({
          slackThreshold: parseInt($('#slackThreshold').value, 10) || 0
        });
        refresh();
      };
      ...
      $('#severityFilter').onchange = () => {
        renderIssues(SM.get(), lastCPMResult);
      };
      $('#filterText').oninput = () => {
        SettingsStore.setFilters({
      ...
      $('#btnBulkReset').onclick = () => {
        ['bulkPhase', 'bulkSub', 'bulkActive', 'bulkDur', 'bulkDurMode', 'bulkPrefix', 'bulkPct', 'bulkAddDep', 'bulkClearDeps', 'bulkShift'].forEach(id => {
      ...
      updateScenarioUI();
      updateBaselineUI();
      $('#scenarioSelect') && ($('#scenarioSelect').onchange = e => {
        SC.switchTo(e.target.value);
        refresh();
      });
      $('#btnScenarioNew') && ($('#btnScenarioNew').onclick = () => {
        const name = prompt('New scenario name');
        if (!name) return;
        SC.saveAs(name);
        refresh();
      });
      $('#btnScenarioSaveAs') && ($('#btnScenarioSaveAs').onclick = () => {
        const name = prompt('Save current as…');
        if (!name) return;
        SC.saveAs(name);
        refresh();
      });
      $('#btnOpenCompare') && ($('#btnOpenCompare').onclick = () => {
        $$('.tab').forEach(x => x.classList.remove('active'));
        $('[data-tab="compare"]').classList.add('active');
        $$('.view').forEach(v => v.classList.remove('active'));
        $('#compare').classList.add('active');
        buildCompare();
      });
      $('#btnUseBaseline') && ($('#btnUseBaseline').onclick = () => {
        const id = $('#baselineSelect').value;
        CURRENT_BASELINE = id;
        buildCompare();
      });
      $('#btnAddBaseline') && ($('#btnAddBaseline').onclick = () => {
      ...
      $('#btnSave').onclick = () => {
        const project = SM.get();
        project.schemaVersion = SCHEMA_VERSION;
        saveFile(JSON.stringify(project, null, 2));
      };
      $('#btnLoad').onclick = async () => {
        ...
        if (errors.length > 0 || warnings.length > 0) {
          let message = '';
          if (errors.length > 0) {
            message += 'Errors found during import:\n' + errors.map(e => "- ".concat(e.msg)).join('\\n');
          }
          if (warnings.length > 0) {
            message += '\\n\\nWarnings:\\n' + warnings.map(w => "- ".concat(w.msg)).join('\\n');
          }
          if (!ok) {
            alert('Import failed!\\n\\n' + message);
            return;
      ...
      (function () {
        let step = 1;
        const max = 5;
        const modal = $('#guided-workflow');
        function showStep() {
          for (let i = 1; i <= max; i++) {
            $('#wz' + i).style.display = i === step ? 'block' : 'none';
          }
          $('#wzPrev').disabled = step === 1;
          $('#wzNext').textContent = step === max ? 'Done' : 'Next';
        }
        $('#wzPrev').onclick = () => {
          if (step > 1) step--;
          showStep();
        ...
        $('#wzNext').onclick = () => {
          if (step < max) step++;else modal.style.display = 'none';
          showStep();
        };
      })();
      ...
      const ZGR = InteractionManager($('#graphSvg'));
      $('#zoomInTL').onclick = ZTL.zoomIn;
      $('#zoomOutTL').onclick = ZTL.zoomOut;
      $('#zoomResetTL').onclick = ZTL.fit;
      ...
      $('#ctxMenu').onclick = e => {
        ...
        if (act === 'edit') {
          ...
          const tok = prompt('Dependency token (e.g. FS:pred+2d)');
          if (tok) {
            const s = SM.get();
            const t = s.tasks.find(x => x.id === id);
            if (t) {
              t.deps = (t.deps || []).concat([tok]);
              SM.replaceTasks(s.tasks, {
                name: 'Add Dependency'
              });
              refresh();
      ...
      $('#action-import-json').addEventListener('click', async () => {
        try {
          ...
          $('#fatal').textContent = "Import Error: ".concat(e.message, ". Please ensure the file is valid JSON.");
      ...
      async function exportGanttToPng() {
        ...
        let css = '';
        const selectors = ['.gantt', '.bar', '.axis', '.label', '.critical', '.milestone', '.progress', '.handle', '.groupHeader', '.groupLabel', '.taskNameBg', '#timeline', '.stripes-bg', '.stripes-line'];
        ...
        img.onload = () => {
          ...
          const a = document.createElement('a');
          a.href = pngUrl;
          a.download = "timeline-export-".concat(todayStr(), ".png");
          document.body.appendChild(a);
          a.click();
          document.body.removeChild(a);
      ...
      $('#action-toggle-theme').addEventListener('click', () => {
        ...
        showToast("Theme switched to ".concat(isDark ? 'Dark' : 'Light', " Mode."));
      });
  ...
  function templateHPC() {
    return ensureIds([{
      id: 'mobo_assembly',
      name: 'Carte mère assemblée (PCB+ASM)',
      duration: w(1),
      subsystem: 'System',
      phase: 'EVT L1'
    }, {
      id: 'mobo_bringup_air',
      name: 'Bring-up carte mère (à air, pas de méca)',
      duration: w(8),
      deps: ['mobo_assembly'],
      subsystem: 'power/VRM',
      phase: 'EVT L1'
    }, {
      id: 'power_bringup_bench',
      name: 'Bring-up power (banc)',
      duration: w(4),
      subsystem: 'power/VRM',
      phase: 'EVT L1'
    }, {
      id: 'power_full_load',
      name: 'Power en puissance',
      duration: w(2),
      deps: ['power_bringup_bench'],
      subsystem: 'power/VRM',
      phase: 'EVT L1'
    }, {
      id: 'power_rails_validated',
      name: 'Power rails validés (gate)',
      duration: d(0),
      deps: ['power_full_load'],
      subsystem: 'power/VRM',
      phase: 'EVT L1'
    }, {
      id: 'bringup_other_cards',
      name: 'Bring-up autres cartes',
      duration: w(4),
      deps: ['mobo_bringup_air'],
      subsystem: 'System',
      phase: 'EVT L1'
    }, {
      id: 'mech_deliveries',
      ...
      id: 'first_blade_assembly',
      name: '1ère lame assembly & corrections méca',
      duration: w(1),
      deps: ['mech_deliveries', 'mobo_bringup_air', 'bringup_other_cards', 'power_full_load'],
      subsystem: 'System',
      phase: 'EVT L1'
    }, {
      id: 'remaining_blades_assembly',
      name: 'Assemblage lames restantes (~5)',
      duration: w(2),
      deps: ['first_blade_assembly'],
      subsystem: 'System',
      phase: 'EVT L1'
    }, {
      id: 'mech_lvl1_lab',
      name: 'Méca L1 labo (WaterBox sans lame)',
      duration: w(1),
      deps: ['mech_deliveries'],
      subsystem: 'Mech',
      phase: 'EVT L1'
    }, {
      id: 'blade_watercool_setup',
      name: 'Setup water cooling',
      duration: w(1),
      deps: ['remaining_blades_assembly', 'mech_lvl1_lab'],
      subsystem: 'Thermal',
      phase: 'EVT L2'
    }, {
      id: 'l2_integration_evt',
      name: 'L2 tests (intégration lame)',
      duration: w(3),
      deps: ['blade_watercool_setup'],
      subsystem: 'System',
      phase: 'EVT L2'
    }, {
      id: 'bmc_fw_evt',
      ...
      name: 'Dev BIOS (EVT)',
      duration: w(4),
      deps: ['power_rails_validated', 'blade_watercool_setup'],
      subsystem: 'BIOS',
      phase: 'EVT L2'
    }, {
      id: 'sw_hal_evt',
      name: 'Dev SW HAL (EVT 20j)',
      duration: d(20),
      deps: ['blade_watercool_setup'],
      subsystem: 'FW',
      phase: 'EVT L2'
      ...
      id: 'evt_done',
      name: 'Fin EVT → GreenLight DVT',
      duration: d(0),
      deps: ['l2_integration_evt'],
      subsystem: 'System',
      phase: 'EVT'
    }, {
      id: 'make_15_blades',
      name: 'Préparer ~15 lames DVT',
      duration: w(4),
      deps: ['evt_done'],
      subsystem: 'System',
      phase: 'DVT L3'
      ...
      id: 'fw_dev_dvt',
      name: 'Dev FW (BMC/BIOS) continue',
      duration: w(4),
      deps: ['make_15_blades'],
      subsystem: 'FW',
      phase: 'DVT L3'
    }, {
      id: 'l2_integration_dvt',
      name: 'L2 intégration + interconnect (8w)',
      duration: w(8),
      deps: ['make_15_blades'],
      subsystem: 'System',
      phase: 'DVT L3'
      ...
      id: 'sw_hal_dvt',
      name: 'Dev SW HAL (DVT 60j)',
      duration: d(60),
      deps: ['make_15_blades', 'sw_hal_evt'],
      subsystem: 'FW',
      phase: 'DVT L3'
    }, {
      id: 'l3_system_test',
      name: 'L3 tests système (8w)',
      duration: w(8),
      deps: ['l2_integration_dvt', 'fw_dev_dvt', 'sw_hal_dvt'],
      subsystem: 'System',
      phase: 'DVT L3'
    }, {
      id: 'blade_for_angers',
      name: 'Lame pour Angers (usine)',
      duration: w(1),
      deps: ['make_15_blades'],
      subsystem: 'System',
      phase: 'DVT L3'
      ...
      id: 'transfer_factory',
      name: 'Transfert à Angers / CdP usine',
      duration: w(1),
      deps: ['dvt_done'],
      subsystem: 'System',
      phase: 'PVT'
      ...
      id: 'train_assembly',
      name: 'Former montage lames',
      duration: w(1),
      deps: ['transfer_factory'],
      subsystem: 'System',
      phase: 'PVT'
    }, {
      id: 'build_card_tester',
      name: 'Construire testeur carte',
      duration: w(3),
      deps: ['transfer_factory'],
      subsystem: 'System',
      phase: 'PVT'
      ...
      id: 'pvt_launch_mfg',
      name: 'Lancer MFG PVT (cartes + méca)',
      duration: w(2),
      deps: ['transfer_factory'],
      subsystem: 'System',
      phase: 'PVT'
...
window.addEventListener('resize', debouncedEnhance, {
  passive: true
});
...
(function () {
  ...
  function setupDropdown(buttonId, menuId, onOpen) {
    ...
    btn.addEventListener('click', () => {
      ...
      exp ? closeMenu() : openMenu();
    });
    document.addEventListener('click', e => {
      const exp = btn.getAttribute('aria-expanded') === 'true';
      if (exp && !menu.contains(e.target) && !btn.contains(e.target)) closeMenu();
    });
