...
function createCPMWorker() {
  ...
  if (!isFile) {
    // normal path for Cloudflare / http(s)
    return new Worker('./assets/js/cpm-worker.js'); // or { type: 'module' } if you use modules
  }
  // file:// fallback — inline the worker source
  const workerSource = `
    // === BEGIN: cpm-worker.js (inlined) ===
'use strict';
...
self.onmessage = function(e) {
  ...
  return new Worker(url); // Blob URL sidesteps file:// origin issues
}
