...
function renderGantt(project, cpm, tickMode='month'){ const svg=$('#gantt'); svg.innerHTML=''; const W=(svg.getBoundingClientRect().width||800); const H=(svg.getBoundingClientRect().height||500); const tasksAll=cpm.tasks.slice(); const tasks=tasksAll.filter(matchesFilters);
  ...
  const startDate=parseDate(project.startDate); const cal=makeCalendar(project.calendar, new Set(project.holidays||[])); const endDate=cal.add(startDate, finish); const gAxis=document.createElementNS('http://www.w3.org/2000/svg','g'); gAxis.setAttribute('class','axis'); const tickArr=[]; if(tickMode==='week'){ let d=new Date(startDate); d.setDate(d.getDate()+((8-d.getDay())%7)); for(; d<=endDate; d=addDays(d,7)){ const offset=cal.diff(startDate,d); if(offset<0) continue; tickArr.push({pos:scale(offset), label:fmtDate(d)}); } } else { const firstLabel=(startDate.getMonth()+1).toString().padStart(2,'0')+'-'+startDate.getFullYear(); tickArr.push({pos:scale(0), label:firstLabel}); let d=new Date(startDate.getFullYear(), startDate.getMonth(),1); d.setMonth(d.getMonth()+1); for(; d<=endDate; d.setMonth(d.getMonth()+1)){ const offset=cal.diff(startDate,d); const label=(d.getMonth()+1).toString().padStart(2,'0')+'-'+d.getFullYear(); tickArr.push({pos:scale(offset), label}); } } for(const tk of tickArr){ const l=document.createElementNS('http://www.w3.org/2000/svg','line'); l.setAttribute('x1',tk.pos); l.setAttribute('y1',20); l.setAttribute('x2',tk.pos); l.setAttribute('y2',chartH-20); l.setAttribute('stroke','#e5e7eb'); gAxis.appendChild(l); const t=document.createElementNS('http://www.w3.org/2000/svg','text'); t.setAttribute('x',tk.pos+2); t.setAttribute('y',14); t.textContent=tk.label; gAxis.appendChild(t); } svg.appendChild(gAxis);
