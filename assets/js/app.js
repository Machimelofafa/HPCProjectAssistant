...
let ganttState = { P: 0, W: 0, finish: 0, cpm: null, svg: null, nodes: new Map() };
let drag = null;
...
function renderGantt(project, cpm){ const svg=$('#gantt'); svg.innerHTML='';
  ganttState.nodes.clear();
  const W=(svg.getBoundingClientRect().width||800); const H=(svg.getBoundingClientRect().height||500); const tasksAll=cpm.tasks.slice(); const tasks=tasksAll.filter(matchesFilters);
  ...
  let y=30; rows.forEach((r)=>{ if(r.type==='group'){ const rect=document.createElementNS('http://www.w3.org/2000/svg','rect'); rect.setAttribute('x',0); rect.setAttribute('y',y-6); rect.setAttribute('width',P-10); rect.setAttribute('height',22); rect.setAttribute('class','groupHeader'); g.appendChild(rect); const tx=document.createElementNS('http://www.w3.org/2000/svg','text'); tx.setAttribute('x',8); tx.setAttribute('y',y+8); tx.setAttribute('class','groupLabel'); tx.textContent=r.label; g.appendChild(tx); y+=22; return; }
    ...
    ganttState.nodes.set(t.id, { bar, label });
...
function onTimelinePointerMove(ev){
  ...
  const gg=ganttState.nodes.get(drag.id)?.bar; if(!gg) return;
...
function onTimelinePointerUp(ev){
  ...
  hideHint();
  const gg=ganttState.nodes.get(drag.id)?.bar;
        ...
        box.onclick = (e) => {
            ...
            if (taskLink) {
              ...
              if (taskId) {
                selectOnly(taskId);
                refresh();
                const bar = ganttState.nodes.get(taskId)?.bar;
                if (bar) {
