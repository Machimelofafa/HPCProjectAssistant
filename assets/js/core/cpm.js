 ...
 * Computes early/late starts/finishes, slack and critical path.
 *
 * @param {Object} project - Project data with tasks, calendar, etc.
 * @returns {Object} {order, tasks, finishDays, warnings}
 */
function computeCPM(project){
  ...
  for(const id of orderRev){
    ...
    for(const arc of succs){
      const s=arc.to; const type=arc.type; const lag=arc.lag|0; const lsS = LS[s]; const lfS = LF[s]; if(lsS==null || lfS==null) continue;
