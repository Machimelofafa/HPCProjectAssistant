...
// Minimal DOM helper used within the store.
...
// ---------------------------------------------------------------------------
// Project state manager with undo/redo and baseline support.
// ---------------------------------------------------------------------------
const SM = (function(){
  ...
  function _apply(next){
    ...
    for(const fn of listeners){
      try{ fn(get()); }catch(e){ console.warn(e); }
    }
  }
  ...
  function set(next, opts={}){
    ...
    if(opts.noSave !== true){ saveState(next); }
    updateUndoUI();
  }
  ...
  function addTasks(list, opts={}){
    ...
    const add=list.map(t=>{ let id=t.id||uid('t'); while(ids.has(id)) id=uid('t'); ids.add(id); return {...t, id}; });
    cur.tasks=cur.tasks.concat(add); set(cur, opts);
  }
  ...
  function updateTask(id, patch, opts={}){
    const cur=get();
    const i=cur.tasks.findIndex(t=>t.id===id); if(i<0) return;
    cur.tasks=cur.tasks.slice(); cur.tasks[i]={...cur.tasks[i], ...patch};
